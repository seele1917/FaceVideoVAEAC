...
# Mask generator for faceVAE
class FaceTimeMaskGenerator:
    def __init__(self, p):
        self.p = p

    def __call__(self, batch):
        nan_mask = torch.isnan(batch).float()  # missed values
        bernoulli_mask_numpy = np.random.choice(2, size=(batch.shape[0], 32, 1),
                                                p=[1 - self.p, self.p])
        bernoulli_mask_numpy = np.tile(bernoulli_mask_numpy,6).reshape(batch.shape[0], -1)
        bernoulli_mask = torch.from_numpy(bernoulli_mask_numpy).float()
        mask = torch.max(bernoulli_mask, nan_mask)  # logical or
        return mask
